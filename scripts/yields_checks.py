...
class YieldsChecks(Tools):
  ...
  def plotSigYields(self, lumi=0.774, selection='', label='', outdirlabel='', category=None):
    ...
    canv = self.tools.createTCanvas('canv', dimx=800, dimy=700)
    canv.SetLogx()
    canv.SetLogy()
    canv.SetGrid()
    ...
    signal_label_m4p5 = 'V42_06Feb23_m4p5_norm'
    ...
    #ctau_points_m1 = [50.0, 70.0, 100.0, 150.0, 200.0, 300.0, 400.0, 500.0, 700.0, 1000.0, 1500.0, 2000.0, 3000.0, 4000.0]
    #ctau_points_m3 = [0.1, 0.15, 0.2, 0.3, 0.4, 0.5, 0.7, 1., 1.5, 2., 3., 4., 5., 7., 10., 15., 20., 30., 40., 50., 70., 100., 150., 200., 300., 400., 500., 700., 1000.] 
    #ctau_points_m3 = [0.2, 0.3, 0.4, 0.5, 0.7, 1., 1.5, 2., 3., 4., 5., 7., 10., 15., 20., 30., 40., 50., 70., 100., 150., 200., 300., 400., 500., 700., 1000.] 
    ctau_points_m3_reweighted = [0.2, 0.3, 0.4, 0.5, 0.7, 1.5, 2., 3., 4., 5., 7., 15., 20., 30., 40., 50., 70., 100., 150.] 
    ...
    ctau_points_m3_generated = [1., 10.] 
    ctau_points_m4p5 = [0.03, 0.04, 0.05, 0.07, 0.1, 0.15, 0.2, 0.3, 0.4, 0.5, 0.7, 1., 1.5, 2., 3., 4., 5., 7., 10., 15., 20., 30., 40., 50., 70., 100.] 
    ...
    graph_m3_generated = ROOT.TGraphAsymmErrors()
    graph_m4p5 = ROOT.TGraphAsymmErrors()

    graph_dummy.SetPoint(0, 1e-5, 1e-2)
    #graph_dummy.SetPoint(0, 1e-6, 1e-0)
    #graph_dummy.SetPoint(0, 1e-6, 1e-0)
    graph_dummy.SetPoint(1, 2e-2, 1e5)
    #graph_dummy.SetPoint(1, 1e-2, 1e5)
    ...
    print '\n mass 3'
    for ctau_point in ctau_points_m3_reweighted:
      # get signal coupling
      signal_mass = 3.
      signal_ctau = ctau_point
      signal_resolution = resolution_p0 + resolution_p1 * signal_mass
      ...
      signal_yields = ComputeYields(signal_label=signal_label_m3, selection=signal_selection).getSignalYields(mass=signal_mass, ctau=signal_ctau, lumi=lumi, sigma_B=472.8e9, is_bc=is_bc, add_weight_hlt=add_weight_hlt, add_weight_pu=add_weight_pu, add_weight_muid=add_weight_muid, weight_hlt=weight_hlt, weight_pusig=weight_pusig, weight_mu0id=weight_mu0id, weight_muid=weight_muid, strategy=strategy)
      ...
      if addBc:
        ...
        signal_yields_bc = ComputeYields(signal_label=signal_label_m3, selection=signal_selection).getSignalYields(mass=signal_mass, ctau=signal_ctau, lumi=lumi, sigma_B=472.8e9, is_bc=True, add_weight_hlt=add_weight_hlt, add_weight_pu=add_weight_pu, add_weight_muid=add_weight_muid, weight_hlt=weight_hlt, weight_pusig=weight_pusig, weight_mu0id=weight_mu0id, weight_muid=weight_muid, strategy=strategy)
        print signal_yields
    ...
    for ctau_point in ctau_points_m3_generated:
      # get signal coupling
      signal_mass = 3.
      signal_ctau = ctau_point
      signal_resolution = resolution_p0 + resolution_p1 * signal_mass
      ...
      # fill graph
      point = graph_m3_generated.GetN()
      graph_m3_generated.SetPoint(point, signal_v2, signal_yields)
      #graph_m3_generated.SetPointError(point, 0, 0, err, err)

    graph_m3_generated.SetMarkerStyle(20)
    graph_m3_generated.SetMarkerSize(2)
    graph_m3_generated.SetMarkerColor(ROOT.kBlue)
    graph_m3_generated.SetLineStyle(line_style)
    graph_m3_generated.SetLineWidth(0)
    graph_m3_generated.SetLineColor(ROOT.kRed+1)
    ...
    #  signal_yields = ComputeYields(signal_label=signal_label_m4p5, selection=signal_selection).getSignalYields(mass=signal_mass, ctau=signal_ctau, lumi=lumi, sigma_B=472.8e9, is_bc=is_bc, add_weight_hlt=add_weight_hlt, add_weight_pu=add_weight_pu, add_weight_muid=add_weight_muid, weight_hlt=weight_hlt, weight_pusig=weight_pusig, weight_mu0id=weight_mu0id, weight_muid=weight_muid, strategy=strategy)
    #  print signal_yields
    #  if addBc:
    #    print '\n Bc'
    ...
    #    signal_yields_bc = ComputeYields(signal_label=signal_label_m4p5, selection=signal_selection).getSignalYields(mass=signal_mass, ctau=signal_ctau, lumi=lumi, sigma_B=472.8e9, is_bc=True, add_weight_hlt=add_weight_hlt, add_weight_pu=add_weight_pu, add_weight_muid=add_weight_muid, weight_hlt=weight_hlt, weight_pusig=weight_pusig, weight_mu0id=weight_mu0id, weight_muid=weight_muid, strategy=strategy)
    ...
    graph_m3_generated.Draw('P same')  
    #graph_m4p5.Draw('PL same')  
    #graph_gen_m1.Draw('PL same')
    #graph_gen_m3.Draw('PL same')
    #graph_gen_m4p5.Draw('PL same')
      
    legend = self.tools.getRootTLegend(xmin=0.15, ymin=0.65, xmax=0.45, ymax=0.85, size=0.038)
    ...
    legend.AddEntry(graph_m3, 'Reweighted points (#it{m}_{N} = 3 GeV)')
    legend.AddEntry(graph_m3_generated, 'Generated points (#it{m}_{N} = 3 GeV)')
